...
DATASET_MEANS = [0.4504, 0.4508, 0.3950]
DATASET_STDS = [0.2210, 0.2149, 0.2380]
MODEL_NEURONS = 16
...
def show_own_predictions(batch_data, model):
    inputs = batch_data
    pred_heatmaps = model(inputs)
    pred_heatmaps = pred_heatmaps.detach().numpy()
    pred_keypoints = heatmaps_to_coordinates(pred_heatmaps)
    
    image_raw = batch_data[0,:, :, :]
    ...
    pred_keypoints_img = pred_keypoints * 128
    
    plt.subplot(2, 1, 2)
    plt.imshow(image_raw)
    for finger, params in COLORMAP.items():
        plt.plot(
            pred_keypoints_img[0][params["ids"], 0],
            pred_keypoints_img[0][params["ids"], 1],
            params["color"],
        )
    plt.title("Pred Keypoints")
    plt.axis("off")
